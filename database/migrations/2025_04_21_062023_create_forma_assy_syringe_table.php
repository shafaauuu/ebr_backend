<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('form_a_assy_syringe', function (Blueprint $table) {
            $table->id();
            $table->string('code_task', 20);
            $table->date('tanggal');
            $table->char('sebelum_produk');
            $table->char('sebelum_bets');
            $table->char('sebelum_needle');
            $table->boolean('bersih_palet');
            $table->boolean('bersih_lantai');
            $table->boolean('bersih_kolong');
            $table->boolean('bersih_mesin');
            $table->boolean('bersih_grill');
            $table->boolean('sisa_lantai');
            $table->boolean('sisa_barrel');
            $table->boolean('sisa_plunger');
            $table->boolean('sisa_needle');
            $table->boolean('sisa_gasket');
            $table->boolean('sisa_starwhell');
            $table->boolean('sisa_box');
            $table->decimal('sisa_reject', 8, 2);
            $table->decimal('sisa_rework', 8, 2);
            $table->boolean('sebelum_dokumen');
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('forma_assy_syringe');
    }
};
