...
use App\Models\FormF\FormFInjection;
use App\Models\Log;
use App\Models\Task;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class FormFInjectionController extends Controller
{
    /**
    ...
    {
        $forms = FormFInjection::with(['task'])->get();
    ...
    public function store(Request $request)
    {
        ...
        \Log::info('Form F Injection Request:', $request->all());
        ...
        try {
            ...
            $form = FormFInjection::create($formData);

            // Log the submission
            Log::create([
                'action' => 'ADD FORM F INJECTION',
                'created_date' => now(),
    ...
    public function show($id, Request $request)
    {
        try {
            ...
            if ($isTaskId) {
                ...
                $forms = FormFInjection::where('task_id', $id)
                    ->with(['task'])
                    ->orderBy('id', 'desc')
                    ->get();
                ...
                if ($forms->isEmpty()) {
                    return response()->json([
                        'message' => 'No Form F Injection records found for this task',
                        'data' => null
                    ], 404);
                }

                return response()->json([
                    'message' => 'Form F Injection records retrieved successfully',
                    'data' => $forms
                ]);
            } else {
                // Get a specific form by its ID
                $form = FormFInjection::with(['task'])->findOrFail($id);

                return response()->json([
                    'message' => 'Form F Injection record retrieved successfully',
                    'data' => $form
                ]);
            }
        } catch (\Exception $e) {
    ...
    {
        // Log the incoming request for debugging
        \Log::info('Form F Injection Update Request:', $request->all());

        $form = FormFInjection::findOrFail($id);
        ...
        try {
            ...
            // Log the update
            Log::create([
                'action' => 'UPDATE FORM F INJECTION',
                'created_date' => now(),
                'created_by' => $request->user() ? $request->user()->nik : 'system',
                'created_at' => now(),
