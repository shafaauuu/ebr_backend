...
use App\Models\FormF\FormFBlister;
use App\Models\Log;
use App\Models\Task;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class FormFBlisterController extends Controller
{
    /**
    ...
    {
        $forms = FormFBlister::with(['task'])->get();
    ...
    public function show($id, Request $request)
    {
        try {
            ...
            if ($isTaskId) {
                ...
                $forms = FormFBlister::where('task_id', $id)
                    ->with(['task'])
                    ->orderBy('id', 'desc')
                    ->get();
                ...
                if ($forms->isEmpty()) {
                    return response()->json([
                        'message' => 'No Form F Blister records found for this task',
                        'data' => null
                    ], 404);
                }

                return response()->json([
                    'message' => 'Form F Blister records retrieved successfully',
                    'data' => $forms
                ]);
            } else {
                // Get a specific form by its ID
                $form = FormFBlister::with(['task'])->findOrFail($id);

                return response()->json([
                    'message' => 'Form F Blister record retrieved successfully',
                    'data' => $form
                ]);
            }
        } catch (\Exception $e) {
    ...
    {
        // Log the incoming request for debugging
        \Log::info('Form F Blister Update Request:', $request->all());
