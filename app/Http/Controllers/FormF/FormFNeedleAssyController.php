...
use App\Models\FormF\FormFNeedleAssy;
use App\Models\Log;
use App\Models\Task;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class FormFNeedleAssyController extends Controller
{
    /**
    ...
    {
        $forms = FormFNeedleAssy::with(['task'])->get();
    ...
    public function store(Request $request)
    {
        ...
        \Log::info('Form F Needle Assy Request:', $request->all());
        ...
        try {
            ...
            $form = FormFNeedleAssy::create($formData);

            // Log the submission
            Log::create([
                'action' => 'ADD FORM F NEEDLE ASSY',
                'created_date' => now(),
    ...
    public function show($id, Request $request)
    {
        try {
            ...
            if ($isTaskId) {
                ...
                $forms = FormFNeedleAssy::where('task_id', $id)
                    ->with(['task'])
                    ->orderBy('id', 'desc')
                    ->get();
                ...
                if ($forms->isEmpty()) {
                    return response()->json([
                        'message' => 'No Form F Needle Assy records found for this task',
                        'data' => null
                    ], 404);
                }

                return response()->json([
                    'message' => 'Form F Needle Assy records retrieved successfully',
                    'data' => $forms
                ]);
            } else {
                // Get a specific form by its ID
                $form = FormFNeedleAssy::with(['task'])->findOrFail($id);

                return response()->json([
                    'message' => 'Form F Needle Assy record retrieved successfully',
                    'data' => $form
                ]);
            }
        } catch (\Exception $e) {
    ...
    {
        // Log the incoming request for debugging
        \Log::info('Form F Needle Assy Update Request:', $request->all());

        $form = FormFNeedleAssy::findOrFail($id);
        ...
        try {
            ...
            // Log the update
            Log::create([
                'action' => 'UPDATE FORM F NEEDLE ASSY',
                'created_date' => now(),
                'created_by' => $request->user() ? $request->user()->nik : 'system',
                'created_at' => now(),
