...
class FormDMachineShi1Controller extends Controller
{
    /**
     * Display machine SHI 1 data based on task ID.
     *
     * @param  int  $taskId
    ...
    {
        try {
            // Find machine SHI 1 data by task ID
            $machineShi1 = MachineShi1::where('task_id', $taskId)
                ->with(['formD', 'task'])
                ->orderBy('id_machine_shi_1', 'desc')
                ->first();

            if (!$machineShi1) {
                return response()->json([
                    'message' => 'No Machine SHI 1 data found for this task',
                    'data' => null
                ], 404);
            }

            return response()->json([
                'message' => 'Machine SHI 1 data retrieved successfully',
                'data' => $machineShi1
            ]);
        } catch (\Exception $e) {
            \Log::error('Error retrieving Machine SHI 1 data: ' . $e->getMessage());
            return response()->json([
                'message' => 'Failed to retrieve Machine SHI 1 data',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    /**
     * Store or update machine SHI 1 data based on task ID.
     *
    ...
    public function store(Request $request)
    {
        ...
        \Log::info('Machine SHI 1 Request:', $request->all());
        ...
        $validator = Validator::make($request->all(), [
            ...
            'temperature_feeder' => 'nullable|numeric',
        ...
        try {
            // Create new record
            $machineShi1 = MachineShi1::create($request->all());
            $action = 'ADD MACHINE SHI 1';
            $message = 'Machine SHI 1 data created successfully';
            ...
            Log::create([
                ...
                'details' => json_encode([
                    'machine_shi_1_id' => $machineShi1->id_machine_shi_1,
            ...
            DB::rollBack();
            \Log::error('Error submitting Machine SHI 1 data: ' . $e->getMessage());
            \Log::error('Error trace: ' . $e->getTraceAsString());

            return response()->json([
                'message' => 'Failed to save Machine SHI 1 data',
                'error' => $e->getMessage()
            ], 500);
        }
    }
