...
class FormDNeedleAssyController
{
    public function getform($taskId)
    {
        try {
            $form_b_data = FormBNeedleAssy::where('task_id', $taskId)->first();
            ...
            \Log::error('Error retrieving Display Machine Needle Assy: ' . $e->getMessage());
            return response()->json([
                'message' => 'Failed to retrieve Display Machine Needle Assy',
                'error' => $e->getMessage()
            ], 500);
        }
    }
