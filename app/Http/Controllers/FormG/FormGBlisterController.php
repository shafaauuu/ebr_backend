...
use App\Models\FormG\FormGBlister;
use App\Models\Log;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class FormGBlisterController extends Controller
{
    /**
    ...
    public function show($id, Request $request)
    {
        try {
            ...
            if ($isTaskId) {
                ...
                $forms = FormGBlister::where('task_id', $id)
                    ->with(['task'])
                    ->orderBy('id', 'desc')
                    ->get();
                ...
                if ($forms->isEmpty()) {
                    return response()->json([
                        'message' => 'No Form G Blister records found for this task',
                        'data' => null
                    ], 404);
                }
                ...
                return response()->json([
                    'message' => 'Form G Blister records retrieved successfully',
                    'data' => $formattedForms
                ]);
            } else {
                // Get a specific form by its ID
                $form = FormGBlister::with(['task'])->findOrFail($id);

                return response()->json([
                    'message' => 'Form G Blister record retrieved successfully',
    ...
    {
        // Log the incoming request for debugging
        \Log::info('Form G Blister Update Request:', $request->except(['signed_1', 'signed_2', 'signed_3']));

        $form = FormGBlister::findOrFail($id);
