...
use App\Models\Bom;
use App\Models\FormC\FormCNeedleAssy;
use App\Models\Log;
...
class FormCNeedleAssyController extends Controller
{
    /**
    ...
    {
        $forms = FormCNeedleAssy::with(['task', 'material', 'brm'])->get();
...
//                ->pluck('childMaterial')
            ...
            $currentData = FormCNeedleAssy::where('task_id', $taskId)
    ...
    public function store(Request $request)
    {
        ...
        \Log::info('Form C Needle Assy Request:', $request->all());
        ...
        try {
            ...
            foreach ($materials as $material) {
                ...
                $form = FormCNeedleAssy::create($formData);
                $createdRecords[] = $form;

                // Log each material submission
                Log::create([
                    'action' => 'ADD FORM C NEEDLE ASSY MATERIAL',
                    'created_date' => now(),
     ...
     *
     * @param int $id The ID of the task or form to retrieve
    ...
    public function show($id, Request $request)
    {
        try {
            ...
            if ($isTaskId) {
                ...
                $forms = FormCNeedleAssy::where('task_id', $id)
                ...
                if ($forms->isEmpty()) {
                    return response()->json([
                        'message' => 'No Form C Needle Assy records found for this task',
                        'data' => null
                    ], 404);
                }
                ...
                $form = FormCNeedleAssy::with(['material', 'task', 'brm'])
                    ->findOrFail($id);
            ...
            \Log::error('Error retrieving Form C Needle Assy: ' . $e->getMessage());
            return response()->json([
    ...
    {
        $materials = FormCNeedleAssy::where('task_id', $taskId)
            ->where('id_mat', $matId)
            ->with(['material'])
    ...
    public function update(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            ...
            'materials.*.id' => 'sometimes|exists:form_c_needle_assy,id',
            'materials.*.batch_no' => 'nullable|string|max:255',
            'materials.*.actual_qty' => 'required|integer|min:0',
        ...
        try {
            ...
            $taskRecord = FormCNeedleAssy::where('task_id', $id)->first();
            ...
            if (!empty($updateData)) {
                ...
                FormCNeedleAssy::where('task_id', $id)->update($updateData);
            }
            ...
            if ($request->has('materials')) {
                foreach ($request->materials as $material) {
                    if (isset($material['id'])) {
                        ...
                        $materialRecord = FormCNeedleAssy::findOrFail($material['id']);
                        ...
                        $newRecord = FormCNeedleAssy::create([
                        ...
                        Log::create([
                            'action' => 'UPDATE FORM C NEEDLE ASSY - ADD MATERIAL',
                            'created_date' => now(),
    ...
    {
        try {
            $material = FormCNeedleAssy::findOrFail($id);
            ...
            // Log the deletion
            Log::create([
                'action' => 'DELETE FORM C NEEDLE ASSY MATERIAL',
                'created_date' => now(),
                'created_by' => request()->user()->nik,
                'created_at' => now(),
                'task_id' => $taskId,
