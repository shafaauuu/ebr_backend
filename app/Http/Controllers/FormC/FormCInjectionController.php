...
//                ->pluck('childMaterial')
            ...
            $currentData = FormCInjection::where('task_id', $taskId)
    ...
    public function show($id, Request $request)
    {
        try {
            ...
            if ($isTaskId) {
                ...
                if ($forms->isEmpty()) {
                    return response()->json([
                        'message' => 'No Form C Injection records found for this task',
                        'data' => null
                    ], 404);
                }
                ...
                $form = FormCInjection::with(['material', 'task', 'brm'])
                    ->findOrFail($id);
            ...
            \Log::error('Error retrieving Form C Injection: ' . $e->getMessage());
            return response()->json([
    ...
    {
        $materials = FormCInjection::where('task_id', $taskId)
            ->where('id_mat', $matId)
            ->with(['material'])
    ...
    public function update(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            ...
            'materials.*.id' => 'sometimes|exists:form_c_injection,id',
            'materials.*.batch_no' => 'nullable|string|max:255',
            'materials.*.actual_qty' => 'required|integer|min:0',
        ...
        try {
            ...
            $taskRecord = FormCInjection::where('task_id', $id)->first();
            ...
            if (!empty($updateData)) {
                ...
                FormCInjection::where('task_id', $id)->update($updateData);
            }
            ...
            if ($request->has('materials')) {
                foreach ($request->materials as $material) {
                    if (isset($material['id'])) {
                        ...
                        $materialRecord = FormCInjection::findOrFail($material['id']);
                        ...
                        $newRecord = FormCInjection::create([
                        ...
                        Log::create([
                            'action' => 'UPDATE FORM C INJECTION - ADD MATERIAL',
                            'created_date' => now(),
    ...
    {
        try {
            $material = FormCInjection::findOrFail($id);
            ...
            // Log the deletion
            Log::create([
                'action' => 'DELETE FORM C INJECTION MATERIAL',
                'created_date' => now(),
                'created_by' => request()->user()->nik,
                'created_at' => now(),
                'task_id' => $taskId,
