...
use App\Models\Bom;
use App\Models\FormC\FormCBlister;
use App\Models\Log;
...
class FormCBlisterController extends Controller
{
    /**
    ...
    {
        $forms = FormCBlister::with(['task', 'material', 'brm'])->get();
...
//                ->pluck('childMaterial')
            ...
            $currentData = FormCBlister::where('task_id', $taskId)
    ...
    public function store(Request $request)
    {
        ...
        \Log::info('Form C Blister Request:', $request->all());
        ...
        try {
            ...
            foreach ($materials as $material) {
                ...
                $form = FormCBlister::create($formData);
                $createdRecords[] = $form;

                // Log each material submission
                Log::create([
                    'action' => 'ADD FORM C BLISTER MATERIAL',
                    'created_date' => now(),
    ...
    public function show($id, Request $request)
    {
        try {
            ...
            if ($isTaskId) {
                ...
                $forms = FormCBlister::where('task_id', $id)
                ...
                if ($forms->isEmpty()) {
                    return response()->json([
                        'message' => 'No Form C Blister records found for this task',
                        'data' => null
                    ], 404);
                }
                ...
                $form = FormCBlister::with(['material', 'task', 'brm'])
                    ->findOrFail($id);
            ...
            \Log::error('Error retrieving Form C Blister: ' . $e->getMessage());
            return response()->json([
    ...
    {
        $materials = FormCBlister::where('task_id', $taskId)
            ->where('id_mat', $matId)
            ->with(['material'])
    ...
    public function update(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            ...
            'materials.*.id' => 'sometimes|exists:form_c_blister,id',
            'materials.*.batch_no' => 'nullable|string|max:255',
            'materials.*.actual_qty' => 'required|integer|min:0',
        ...
        try {
            ...
            $taskRecord = FormCBlister::where('task_id', $id)->first();
            ...
            if (!empty($updateData)) {
                ...
                FormCBlister::where('task_id', $id)->update($updateData);
            }
            ...
            if ($request->has('materials')) {
                foreach ($request->materials as $material) {
                    if (isset($material['id'])) {
                        ...
                        $materialRecord = FormCBlister::findOrFail($material['id']);
                        ...
                        Log::create([
                            'action' => 'UPDATE FORM C BLISTER - ADD MATERIAL',
                            'created_date' => now(),
    ...
    {
        try {
            $material = FormCBlister::findOrFail($id);
            ...
            // Log the deletion
            Log::create([
                'action' => 'DELETE FORM C BLISTER MATERIAL',
                'created_date' => now(),
                'created_by' => request()->user()->nik,
                'created_at' => now(),
                'task_id' => $taskId,
